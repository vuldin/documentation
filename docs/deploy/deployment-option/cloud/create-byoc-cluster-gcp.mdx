...
12. Under **Choose Connection Type**, select either **Public** or **Private**. For BYOC clusters, **Private** is best-practice.
Your network name is used in the Redpanda UI to identify this network. Choose a CIDR range that does not overlap with your existing GCP VPCs or your Redpanda network. 
...
[Create a VPC Peering Connection to a GCP BYOC Cluster](vpc-peering-gcp.mdx)
=======
  ```shell
  rpk cloud byoc gcp apply
    --redpanda-id='...'
...
Note that `rpk` configures the permissions required by the agent to provision and
actively maintain the cluster. For details about these permissions, see
[GCP IAM Permissions](../security/authorization/cloud-iam-policies#gcp-iam-permissions).
