...
:::tip
This guide uses `jq` to make parsing JSON output easier. If you want more details, see [jq downloads](https://stedolan.github.io/jq/download/).
:::

## Should you decommission brokers?

There are several considerations for determining your cluster's minimum broker count, and whether or not to decommission any brokers. For the purposes of this section, the focus is on a cluster with 7 brokers. In subsequent sections, the output from the given commands provides additional details to help you determine the minimum number of brokers.
...
Rack awareness is just one aspect of availability. Check out [High Availability](https://docs.redpanda.com/docs/deploy/deployment-option/self-hosted/manual/high-availability/) for more details on deploying Redpanda for high availability.
...
Local data retention is determined by the storage capability of each broker and how much data is being produced over a given period (that is, producer throughput). When decommissioning, storage capability must take into account both the free storage space as well as the amount of space already used by existing partitions.
...
Cost and use case requirements will dictate how much to spend on local disk capacity. Tiered storage can help to both decrease costs and expand data retention capabilities. For details, see [Tiered Storage](https://docs.redpanda.com/docs/manage/tiered-storage/).

:::note
At this point in the example, it remains unclear that we can scale down to 5 brokers. Current calculations are based on 5 brokers. Other broker counts can be used later as needed.

Additionally, some assumptions have been made regarding a constant throughput and perfect data balancing. In reality, throughput fluctuates across all partitions, which causes data imbalance. The calculations above attempt to accommodate for this by padding disk size by 1%. You can increase this buffer (for example, in the case of expected hot spot partitions). For details on sizing, see [Sizing Guidelines](https://docs.redpanda.com/docs/deploy/deployment-option/self-hosted/manual/sizing/).
:::
...
In general, a cluster will withstand a higher number of brokers going down if there are more brokers in the cluster. For details, see [Raft consensus algorithm](https://docs.redpanda.com/docs/get-started/architecture/#raft-consensus-algorithm).
...
In this example each broker has 12 cores available. If you plan to scale down the number of brokers to 5, then you would have 40 cores available, which means that your cluster is limited by core count to 40K partitions (well above the current 3018 partitions).
...
The following details summarize the considerations tested above:

- At least 4 brokers are required based on availability.
- Cost is not a limiting factor in this example, but less cost (and lower broker count) is always best.
- At least 1.2TB of data resides on each broker (if spread across 5 brokers). This falls within the 1.5TB of local storage available in the example.
- At least 5 brokers are required based on the highest replication factor across all topics.
...
So the primary limitation consideration is the replication factor of 5, meaning that you could scale down to 5 brokers at minimum.

## Decommission a broker

After going through the process above, you should now know how many brokers you can scale down to. In our example, we have 7 brokers and we will scale down to 5.
...
The output shows that the IDs don't match the statefulset ordinal, which appears in the hostname. We will be decommissioning two brokers: `redpanda-6` (ID 8) and `redpanda-5` (ID 9).

:::note
When decommissioning brokers, you may initially want to selectively decommission brokers (possibly to keep brokers assigned to different racks or some other reason). But this would be an issue since Redpanda is deployed within a statefulset on Kubernetes. The statefulset will control which pods are destroyed and will always start with the highest ordinal. In our example, the statefulset ordinals match the number in the hostname. So the first broker that will be destroyed when updating the statefulset is ID 8 (or `redpanda-6.testcluster.local`). A better approach is to reassign the remaining brokers to appropriate racks after the decommission process is complete.
:::
...
1. Decommission the broker with your selected broker ID:
  ...
  :::note
  The above command will return before the decommission process is complete.
  :::
...
1. Monitor the decommissioning status:
  ...
  You want to verify that your decommissioned broker's ID does not appear in the list of IDs. In the above example, ID 9 is missing (which means the decommission is complete).

1. Decommission any other brokers

  Now that we have decommissioned one broker and verified the process is complete, we can either continue decommissioning another broker (by repeating the previous two steps) or continue to the next step. In our example we were decommissioning two brokers (8 and 9), so we will repeat the previous two steps. You can do the same as many times as needed to get to the number of desired brokers.

  :::note
  Make sure you have taken into account everything in [this section](#should-you-decommission-brokers), and that you have verified your cluster and your use cases will not be negatively impacted by losing brokers.
  :::

1. Update statefulset replica value

  The last step is to update the statefulset replica value to reflect the new broker count. In our example this means we update the count to 5. If you deployed with the helm chart, then the following command will work:

  ```bash
  helm upgrade redpanda redpanda/redpanda -n redpanda --wait --reuse-values --set statefulset.replicas=5
  ```

  This will trigger a rolling restart of each pod, which is required since each broker has its configuration (`seed_servers`) updated to reflect the new broker list.
